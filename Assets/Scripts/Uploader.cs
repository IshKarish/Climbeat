...
public class Uploader : MonoBehaviour
{
    ...
    {
        //FileBrowser.SetFilters(false, new FileBrowser.Filter("Not Virus", ".fuckunity"));
    ...
    IEnumerator ShowinfoDialog()
    {
        ...
        if (FileBrowser.Success)
        {
            ...
            StartCoroutine(ShowStealDialog(bpm));
        }
    }

    IEnumerator ShowStealDialog(int bpm)
    {
        yield return FileBrowser.WaitForLoadDialog(FileBrowser.PickMode.FilesAndFolders);
        ...
        if (FileBrowser.Success)
        {
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                ...
                secsLst.Add(float.Parse(str) / bpm * 60);
            }
    ...
    IEnumerator ShowContDialog()
    {
        ...
        if (FileBrowser.Success)
        {
            ...
            path = Path.Combine(FileBrowserHelpers.GetDirectoryName(res), FileBrowserHelpers.GetFilename(res)) + "/Normal.fuckunity";
            Debug.Log(path);
